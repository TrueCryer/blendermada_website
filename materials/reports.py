...
class StatisticReport(object):
    ...
    def get_html_rolling_mean(self):
        return pd.rolling_apply(
            self.all_by_day['count'], 60, np.mean,
        )[-15:].to_frame().to_html()

    def get_html_count_by_month(self):
        return self.all_by_day.resample('M').sum()[-6:].to_html()
    ...
    def get_svg_count_by_day(self):
        ...
        df_mean_30 = self.all_by_day['count'].rolling(
            window=30, center=False).apply(func=np.mean)[-15:].to_frame()
        df_mean_60 = self.all_by_day['count'].rolling(
            window=60, center=False).apply(func=np.mean)[-15:].to_frame()
    ...
    def get_svg_count_by_month(self):
        df = self.all_by_day.resample('M').sum()[-10:]
        chart = pygal.Bar(
            width=960, height=480,
            explicit_size=True,
            show_legend=False
